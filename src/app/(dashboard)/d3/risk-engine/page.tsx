  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-white font-['Space_Grotesk'] tracking-tight">Risk Score Engine</h1>
          <p className="text-gray-400">Neural-network based threat assessment and case prioritization.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Threat Gauge */}
        <Card className="lg:col-span-1 bg-[#111827] border-white/5 relative overflow-hidden flex flex-col items-center justify-center p-8">
           ...
           <div className="relative w-64 h-64 flex items-center justify-center">
              ...
              <div className="absolute flex flex-col items-center text-center">
                 ...
                 <span className="text-[10px] font-bold text-purple-400 uppercase tracking-[0.2em] mt-2">Threat Index</span>
              </div>
           </div>

           <div className={cn("mt-8 flex items-center gap-2 px-4 py-2 rounded-full border",
             score > 75 ? "bg-red-500/10 border-red-500/20" : score > 50 ? "bg-amber-500/10 border-amber-500/20" : "bg-green-500/10 border-green-500/20"
           )}>
              <ShieldAlert className={cn("w-4 h-4", score > 75 ? "text-red-500" : score > 50 ? "text-amber-500" : "text-green-500")} />
              <span className={cn("text-xs font-bold uppercase", score > 75 ? "text-red-400" : score > 50 ? "text-amber-400" : "text-green-400")}>
        ...
        <div className="lg:col-span-2 space-y-6">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {riskFactors.map((factor, i) => (
                <Card key={i} className="bg-[#111827] border-white/5 group hover:border-purple-500/30 transition-all">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-xs text-gray-500 font-bold uppercase tracking-widest">{factor.label}</span>
                      <span className={cn("text-[10px] font-bold px-1.5 py-0.5 rounded",
                        factor.impact === "Critical" ? "bg-red-500/10 text-red-400" : "bg-purple-500/10 text-purple-400"
                      )}>{factor.impact} IMPACT</span>
                    </div>
                    <div className="flex items-end justify-between">
                      <span className="text-2xl font-bold text-white font-mono">{factor.value}</span>
                      {factor.trend === "up" && <TrendingUp className="w-4 h-4 text-red-400" />}
                    </div>
                  </CardContent>
                </Card>
              ))}
           ...
           <Card className="bg-[#111827] border-white/5">
             <CardHeader>
                <CardTitle className="text-lg text-white flex items-center gap-2 font-['Space_Grotesk']">
                  ...
                  {aiLoading && <Loader2 className="w-4 h-4 animate-spin text-purple-400 ml-2" />}
                </CardTitle>
             </CardHeader>
             <CardContent>
                <div className="space-y-4">
                   {aiLogic.length > 0 ? aiLogic.map((logic, i) => (
                     <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/5">
                        ...
                        <p className="text-xs text-gray-400 leading-relaxed">{logic}</p>
                     </div>
                   )) : (
                     <p className="text-xs text-gray-600 text-center py-8">Click "Reassess Risk" to generate AI intelligence.</p>
                   )}
                </div>
             </CardContent>
           </Card>
