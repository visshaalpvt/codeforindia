...
import { FileText, Plus, Search, Clock, MapPin, User, Download, X } from "lucide-react";
...
export default function FieldReportsPage() {
  ...
  const [reports, setReports] = useState<{ id: string; title: string; officer: string; date: string; time: string; location: string; type: string; priority: string; body: string; caseId: string }[]>([]);
  const [newReport, setNewReport] = useState({ title: "", officer: "", location: "", type: "Incident", priority: "Medium", body: "", caseId: "" });
  const [searchTerm, setSearchTerm] = useState("");

  const addReport = () => {
    if (!newReport.title || !newReport.officer || !newReport.body) return;
    const now = new Date();
    setReports(prev => [...prev, {
      id: `FR-${now.getFullYear()}-${String(prev.length + 1).padStart(3, "0")}`,
      title: newReport.title,
      officer: newReport.officer,
      date: now.toISOString().split("T")[0],
      time: now.toTimeString().slice(0, 5),
      location: newReport.location || "Unspecified",
      type: newReport.type,
      priority: newReport.priority,
      body: newReport.body,
      caseId: newReport.caseId || cases[0]?.id || "Unlinked",
    }]);
    setNewReport({ title: "", officer: "", location: "", type: "Incident", priority: "Medium", body: "", caseId: "" });
    setShowAdd(false);
  };

  const exportReport = (r: typeof reports[0]) => {
    const content = generateForensicReport(`Field Report — ${r.id}`, [
      { heading: "Report Details", content: `  Report ID: ${r.id}\n  Title: ${r.title}\n  Officer: ${r.officer}\n  Date: ${r.date} at ${r.time}\n  Location: ${r.location}\n  Type: ${r.type}\n  Priority: ${r.priority}\n  Linked Case: ${r.caseId}` },
      { heading: "Report Body", content: r.body },
    ]);
    downloadReport(`${r.id}_Field_Report.txt`, content);
  };

  const filtered = reports.filter(r =>
    !searchTerm || r.title.toLowerCase().includes(searchTerm.toLowerCase()) || r.officer.toLowerCase().includes(searchTerm.toLowerCase()) || r.id.toLowerCase().includes(searchTerm.toLowerCase())
  );
                ...
                <CardContent className="p-0">
                  <div className="flex items-stretch">
                    <div className={cn("w-2",
                      ...
                      report.priority === "Medium" ? "bg-blue-500" : "bg-gray-500"
                    )} />
