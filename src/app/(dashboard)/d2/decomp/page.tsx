...
const decompStages = [
  { id: 1, name: "Fresh", range: "0-2 days", desc: "No visible decomposition. Autolysis begins internally.", color: "text-green-600", bg: "bg-green-500/10" },
  ...
  { id: 3, name: "Active Decay", range: "6-10 days", desc: "Mass loss through purging fluids and insect activity.", color: "text-amber-600", bg: "bg-amber-50" },
  ...
  { id: 5, name: "Skeletonization", range: "25+ days", desc: "Only skeletal remains, cartilage, and dried skin remain.", color: "text-red-600", bg: "bg-red-500/10" },
];
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 font-['Space_Grotesk'] tracking-tight">Decomp Tracker</h1>
          <p className="text-slate-500">5-stage decomposition tracking with biochemical marker analysis.</p>
        </div>
        <select
          value={selectedCase}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="bg-white border-white/5">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-400 font-bold uppercase mb-1">Scene Temperature</p>
              <p className="text-3xl font-bold text-slate-900 font-mono">{Number(currentTemp).toFixed(1)}°C</p>
              <p className="text-[10px] text-slate-400 mt-1">From IoT sensor feed</p>
            </div>
            <Thermometer className="w-10 h-10 opacity-20 text-amber-600" />
          </CardContent>
        </Card>
        <Card className="bg-white border-white/5">
          <CardContent className="p-6 flex items-center justify-between">
            <div>
              <p className="text-xs text-slate-400 font-bold uppercase mb-1">Relative Humidity</p>
              <p className="text-3xl font-bold text-slate-900 font-mono">{Number(currentHumidity).toFixed(0)}%</p>
              <p className="text-[10px] text-slate-400 mt-1">From IoT sensor feed</p>
            </div>
            <Droplets className="w-10 h-10 opacity-20 text-blue-600" />
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {decompStages.map((stage) => (
              ...
              >
                <div className={cn("text-2xl font-bold font-mono mb-2", selectedStage === stage.id ? stage.color : "text-slate-400")}>{stage.id}</div>
                <p className={cn("text-sm font-bold mb-1", selectedStage === stage.id ? "text-slate-900" : "text-slate-500")}>{stage.name}</p>
                <p className="text-[10px] text-slate-400">{stage.range}</p>
              </motion.button>
            ))}
          </div>
        </CardContent>
        ...
        <CardContent>
          {loading ? (
            <div className="flex flex-col items-center justify-center py-16 gap-4">
              ...
              <p className="text-sm text-slate-400">Analyzing {decompStages.find(s => s.id === selectedStage)?.name} stage...</p>
            </div>
          ) : analysis ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <p className="text-sm text-slate-400">Select a decomposition stage to analyze</p>
            </div>
          )}
        </CardContent>
      </Card>
