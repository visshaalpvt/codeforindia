  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 font-['Space_Grotesk'] tracking-tight">Toxicology Panel</h1>
          <p className="text-slate-500">Chemical analysis and drug screening for biological samples.</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={runAIAnalysis}
            disabled={aiLoading}
            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-50 border border-amber-500/20 text-amber-600 font-bold text-sm hover:bg-amber-100 transition-all disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          { label: "Active Screens", value: String(activeScreens).padStart(2, "0"), icon: FlaskConical, color: "text-amber-600" },
          { label: "Critical Findings", value: String(criticalFindings).padStart(2, "0"), icon: ShieldAlert, color: "text-red-600" },
          { label: "Pending Labs", value: String(cases.filter(c => c.status === "Active").length), icon: Clock, color: "text-blue-600" },
          { label: "Completed (24h)", value: String(evidence.filter(e => e.custodyStatus === "In Lab").length), icon: CheckCircle2, color: "text-green-600" },
        ].map((stat, i) => (
      ...
      <Card className="bg-white border-white/5">
        <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 pb-4">
          <div className="flex items-center gap-4">
            <button onClick={() => setActiveTab("active")} className={cn("text-sm font-bold transition-all px-2 py-1", activeTab === "active" ? "text-amber-600 border-b-2 border-amber-400" : "text-slate-400")}>Active Screens</button>
            <button onClick={() => setActiveTab("history")} className={cn("text-sm font-bold transition-all px-2 py-1", activeTab === "history" ? "text-amber-600 border-b-2 border-amber-400" : "text-slate-400")}>All Evidence</button>
          </div>
        </CardHeader>
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left border-b border-white/5">
                  <th className="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Evidence ID</th>
              ...
              <tbody className="divide-y divide-white/5">
                {(activeTab === "active" ? toxEvidence : evidence).map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 hover:bg-slate-100 transition-colors">
                    <td className="px-6 py-4 text-xs font-mono text-amber-600">{item.id}</td>
                    <td className="px-6 py-4 text-sm font-bold text-slate-900">{item.name}</td>
                    <td className="px-6 py-4 text-sm text-slate-700">{item.type}</td>
                    <td className="px-6 py-4">
                      <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border",
                        item.custodyStatus === "In Lab" ? "bg-green-500/10 text-green-600 border-green-500/20" :
                        "bg-amber-50 text-amber-600 border-amber-500/20"
                      )}>
                        {item.custodyStatus}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-xs font-bold text-slate-500">{item.caseId}</td>
                  </tr>
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="bg-white border-white/5">
          ...
          <CardContent>
            <div className="h-48 w-full bg-slate-50 hover:bg-slate-100 rounded-xl border border-white/5 relative overflow-hidden p-4">
               <div className="absolute inset-0 flex items-end justify-around px-4">
                 {[10, 40, 15, 80, 20, 35, 95, 25, 45, 60, 10, 15, 30].map((h, i) => (
                   ...
                   >
                     <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-[8px] font-mono text-amber-600 opacity-0 group-hover:opacity-100 transition-opacity">
                       {(h * 12.4).toFixed(0)}
                     </div>
                   </motion.div>
                 ))}
        ...
        <Card className="bg-white border-white/5">
          ...
          <CardContent className="space-y-4">
             <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 hover:bg-slate-100 border border-white/5">
               <div className="flex items-center gap-3">
                 <Zap className="w-4 h-4 text-green-600" />
                 <span className="text-sm text-slate-900">Centrifuge #1</span>
               </div>
             ...
             <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 hover:bg-slate-100 border border-white/5">
               <div className="flex items-center gap-3">
                 <Zap className="w-4 h-4 text-green-600" />
                 <span className="text-sm text-slate-900">Sequence Bot-7</span>
               </div>
               <span className="text-[10px] font-bold text-green-600 uppercase">Operational</span>
             </div>
             <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 hover:bg-slate-100 border border-white/5 opacity-50">
               <div className="flex items-center gap-3">
                 <Zap className="w-4 h-4 text-slate-400" />
                 <span className="text-sm text-slate-900">GC-MS Scanner</span>
               </div>
